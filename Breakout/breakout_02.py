...
config_dict["minibatch_size"] = int(
    config_dict["batch_size"] // config_dict["n_minibatches"]
)
...
# Set name and initialize writer
...
envs = gym.vector.SyncVectorEnv(
    [make_env(config.gym_id, i, config.run_name) for i in range(config.n_envs)]
)
...
class PPONetwork(nn.Module):
    def __init__(self, input_shape, n_actions, hidden_size=512):
        ...
        self.critic = nn.Sequential(layer_init(nn.Linear(hidden_size, 1), std=1.0))
...
states = torch.zeros(
    (config.n_steps, config.n_envs) + envs.single_observation_space.shape
).to(config.device)
...
)  # Total number of learning update that will be performed
...
for update in range(1, num_updates + 1):
    ...
    for step in range(0, config.n_steps):
        ...
        if "final_observation" in info.keys():
            for i_env, done_flag in enumerate(info["_final_observation"]):
                if done_flag:
                    print(
                        f"\rglobal_step={global_step}, episodic_return={episodic_return[i_env]}",
                        end="",
                    )

                    writer.add_scalar(
                        "charts/episodic_return", episodic_return[i_env], global_step
                    )
                    writer.add_scalar(
                        "charts/episodic_length", episode_step_count[i_env], global_step
                    )

                    episodic_return[i_env], episode_step_count[i_env] = 0.0, 0.0
        ...
        if config.gae:
            ...
            for t in reversed(range(config.n_steps)):
                ...
                advantages[t] = lastgaelam = (
                    delta
                    + config.gamma * config.gae_lambda * next_non_terminal * lastgaelam
                )
    ...
    for epoch in range(config.update_epochs):
        ...
        for start in range(0, config.batch_size, config.minibatch_size):
            ...
            _, newlogprob, entropy, newvalue = network(
            ...
            loss_surrogate_clipped = -mb_advantages * torch.clip(
            ...
            loss = (
                loss_policy
                + config.weight_ent_loss * -loss_entropy
                + config.weight_value_loss * loss_value
            )
...
def play_game(env, network, n_episodes=1):
    for episode in range(n_episodes):
        state, _ = env.reset()
        done = False
        total_reward = 0
        while not done:
            # Ensure state is a NumPy array and handle LazyFrames
            ...
            action = action.cpu().numpy().item()  # Ensure action is a scalar

            state, reward, terminated, truncated, _ = env.step(action)

            done = terminated or truncated
            total_reward += reward
            env.render()  # Render the environment to view the gameplay
        print(f"Episode {episode + 1}: Total Reward = {total_reward}")


# Create a new environment for playing
play_env = make_env(config.gym_id, 0, config.run_name)()

# ------------------------------------------------
# Record video of the gameplay
# ------------------------------------------------
