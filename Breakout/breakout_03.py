...
size_replay_memory = (
    100000  # メモリのサイズ ※Google ColabなどRAMサイズが小さい場合は10000程度にする
)
...
def to_resize_gray(image, resize):
    ...
    image = (
        cv2.resize(src=image, dsize=(resize, resize)) / 255.0
    )  # 画像のリサイズと値の変換0～1
...
for i_episode in tqdm(range(num_episodes)):
    ...
    if terminated == True:
        state_frame = torch.zeros(
            (1, n_frame, resize_image, resize_image), dtype=torch.float32
        )
        next_state_frame = torch.zeros(
            (1, n_frame, resize_image, resize_image), dtype=torch.float32
        )
        state, info = env.reset(seed=random.randint(0, 2**24))
        state = to_resize_gray(state, resize_image)
    ...
    for t in count():
        ...
        if (
            old_life > info["lives"]
        ):  # ライフが減った場合にtruncatedをTrueにして次エピソードにする
            old_life = info["lives"]
            truncated = True
    ...
    if i_episode % num_episode_plot == 0:
        ...
        with out:
            ...
            plt.plot(
                np.arange(
                    0,
                    (i_episode / num_episode_plot + 1) * num_episode_plot,
                    num_episode_plot,
                ),
